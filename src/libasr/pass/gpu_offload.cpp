...
// Recursively check if a Struct has any allocatable sub-members at any
// nesting depth through non-allocatable struct intermediaries.
static bool struct_has_allocatable_deep(ASR::Struct_t *st) {
    for (size_t m = 0; m < st->n_members; m++) {
        ASR::symbol_t *ms = st->m_symtab->get_symbol(st->m_members[m]);
        if (!ms || !ASR::is_a<ASR::Variable_t>(*ms)) continue;
        ASR::Variable_t *mv = ASR::down_cast<ASR::Variable_t>(ms);
        if (ASRUtils::is_allocatable(mv->m_type)) return true;
        ASR::ttype_t *mt = ASRUtils::extract_type(mv->m_type);
        ...
        if (struct_has_allocatable_deep(
...
// Check if a struct has allocatable sub-members reachable only through
// non-allocatable struct intermediaries. Direct allocatable members
// don't count (they are handled by the flat buffer decomposition).
// Used to decide whether a struct-to-struct assignment needs to be
// decomposed into per-member assignments in the GPU kernel.
static bool struct_needs_assignment_decomposition(ASR::Struct_t *st) {
    for (size_t m = 0; m < st->n_members; m++) {
        ASR::symbol_t *ms = st->m_symtab->get_symbol(st->m_members[m]);
        if (!ms || !ASR::is_a<ASR::Variable_t>(*ms)) continue;
        ASR::Variable_t *mv = ASR::down_cast<ASR::Variable_t>(ms);
...
// Recursively emit pre-allocation DoLoops for allocatable members found
// at any depth through non-allocatable struct intermediaries.
// chain_exts/chain_types: accumulated (ExternalSymbol, type) pairs from
// the outermost non-allocatable struct member down to the current level.
// src_chain_exts/src_chain_types: same chain but for building the source
// expression from ctor_arg.
static void emit_prealloc_deep(
    Allocator &al,
    const Location &loc,
    SymbolTable *current_scope,
    ASR::Struct_t *st,
    ASR::expr_t *ctor_arg,
    const ASR::DoConcurrentLoop_t &x,
    ASR::symbol_t *tgt_sym,
    ASR::ttype_t *elem_type,
    ASR::symbol_t *outer_mem_ext,
    ASR::ttype_t *outer_mem_type,
    std::vector<std::pair<ASR::symbol_t*, ASR::ttype_t*>> &chain,
    Vec<ASR::stmt_t*> &pre_launch_stmts)
{
    ASR::ttype_t *int_type = ASRUtils::TYPE(
        ASR::make_Integer_t(al, loc, 4));

    for (size_t sm = 0; sm < st->n_members; sm++) {
        ASR::symbol_t *sub_sym =
            st->m_symtab->get_symbol(st->m_members[sm]);
        ...
        for (auto &scope_item : current_scope->get_scope()) {
        ...
        if (ASRUtils::is_allocatable(sub_mv->m_type)) {
            ASR::ttype_t *sub_mi =
                ASRUtils::type_get_past_allocatable(sub_mv->m_type);
            if (!ASR::is_a<ASR::Array_t>(*sub_mi)) continue;

            // Build source expression: ctor_arg %chain... %sub_member
            ASRUtils::ExprStmtDuplicator dup_sz(al);
            dup_sz.success = true;
            ASR::expr_t *src_expr =
                dup_sz.duplicate_expr(ctor_arg);
            for (auto &cp : chain) {
                src_expr = ASRUtils::EXPR(
                    ASR::make_StructInstanceMember_t(al, loc,
                        src_expr, cp.first, cp.second, nullptr));
            }
            src_expr = ASRUtils::EXPR(
                ASR::make_StructInstanceMember_t(al, loc,
                    src_expr, sub_ext, sub_mv->m_type, nullptr));
            ASR::expr_t *sub_size = ASRUtils::EXPR(
                ASR::make_ArraySize_t(al, loc,
                    src_expr, nullptr, int_type, nullptr));

            // Build DoLoop for pre-allocation
            ...
            idx[0].loc = loc;
            idx[0].m_left = nullptr;
            idx[0].m_right = lv;
            idx[0].m_step = nullptr;

            // Build target: dst(k) %outer_mem %chain... %sub_member
            ...
            ASR::expr_t *tgt_expr = ASRUtils::EXPR(
                ASR::make_StructInstanceMember_t(al, loc,
                    tgt_elem, outer_mem_ext,
                    outer_mem_type, nullptr));
            for (auto &cp : chain) {
                tgt_expr = ASRUtils::EXPR(
                    ASR::make_StructInstanceMember_t(al, loc,
                        tgt_expr, cp.first, cp.second, nullptr));
            }
            tgt_expr = ASRUtils::EXPR(
                ASR::make_StructInstanceMember_t(al, loc,
                    tgt_expr, sub_ext,
            ...
            ASR::expr_t *tgt_dup =
                dup_lp.duplicate_expr(tgt_expr);
            ASR::ttype_t *log_type = ASRUtils::TYPE(
                ASR::make_Logical_t(al, loc, 4));
            Vec<ASR::expr_t*> aa_args;
            aa_args.reserve(al, 1);
            aa_args.push_back(al, tgt_dup);
            ASR::expr_t *is_alloc = ASRUtils::EXPR(
                ASR::make_IntrinsicImpureFunction_t(al, loc,
                    ...
                    aa_args.p, aa_args.n,
                    0, log_type, nullptr));
            ASR::expr_t *not_alloc = ASRUtils::EXPR(
                ASR::make_LogicalNot_t(al, loc,
                    is_alloc, log_type, nullptr));

            Vec<ASR::stmt_t*> if_body;
            if_body.reserve(al, 1);
            if_body.push_back(al, alloc_s);

            ASR::stmt_t *guard = ASRUtils::STMT(
            ...
            // Non-allocatable struct member: recurse deeper
            ASR::ttype_t *mt =
                ASRUtils::extract_type(sub_mv->m_type);
            if (!ASR::is_a<ASR::StructType_t>(*mt)) continue;
            if (!sub_mv->m_type_declaration) continue;
            ASR::symbol_t *deeper_sym =
                ASRUtils::symbol_get_past_external(
                    sub_mv->m_type_declaration);
            if (!ASR::is_a<ASR::Struct_t>(*deeper_sym)) continue;
            chain.push_back({sub_ext, sub_mv->m_type});
            emit_prealloc_deep(al, loc, current_scope,
                ASR::down_cast<ASR::Struct_t>(deeper_sym),
                ctor_arg, x, tgt_sym, elem_type,
                outer_mem_ext, outer_mem_type,
...
// Like emit_prealloc_deep but builds the target through an allocatable
// array member with a dynamic inner loop.  Generates:
//   do i = dc_start, dc_end
//     do j = 1, size(arr(i) % alloc_member)
//       if (.not. allocated(arr(i)%alloc_member(j)%sub%chain%leaf))
//         allocate(arr(i)%alloc_member(j)%sub%chain%leaf(size(src%chain%leaf)))
//     end do
//   end do
static void emit_prealloc_deep_through_alloc_member(
    Allocator &al,
    const Location &loc,
    SymbolTable *current_scope,
    ASR::Struct_t *st,
    ASR::expr_t *sub_ctor_arg,
    const ASR::DoConcurrentLoop_t &x,
    ASR::symbol_t *tgt_sym,
    ASR::ttype_t *tgt_elem_type,
    ASR::symbol_t *alloc_mem_ext,
    ASR::ttype_t *alloc_mem_type,
    ASR::symbol_t *sub_mem_ext,
    ASR::ttype_t *sub_mem_type,
    std::vector<std::pair<ASR::symbol_t*, ASR::ttype_t*>> &chain,
    Vec<ASR::stmt_t*> &pre_launch_stmts)
{
    ASR::ttype_t *int_type = ASRUtils::TYPE(
    ...
    for (size_t sm = 0; sm < st->n_members; sm++) {
        ...
        if (!sub_ext) {
            std::string es_nm =
                current_scope->get_unique_name(
                    ...
                    std::string(st->m_members[sm]), false);
            ASR::asr_t *new_es =
                ASR::make_ExternalSymbol_t(al, loc,
                    current_scope, s2c(al, es_nm),
                    sub_sym, s2c(al, st->m_name),
                    nullptr, 0,
                    s2c(al, st->m_members[sm]),
                    ASR::accessType::Public);
            sub_ext = ASR::down_cast<ASR::symbol_t>(new_es);
            current_scope->add_symbol(es_nm, sub_ext);
        }
        ...
        if (ASRUtils::is_allocatable(sub_mv->m_type)) {
            ...
            // Source: sub_ctor_arg %chain %sub_member
            ASRUtils::ExprStmtDuplicator dup_sz(al);
            dup_sz.success = true;
            ASR::expr_t *src_expr =
                dup_sz.duplicate_expr(sub_ctor_arg);
            for (auto &cp : chain) {
                src_expr = ASRUtils::EXPR(
                    ASR::make_StructInstanceMember_t(al, loc,
                        src_expr, cp.first, cp.second, nullptr));
            ...
            // Create inner loop variable j
            std::string jvar_name =
                current_scope->get_unique_name(
                    "__prealloc_j", false);
            ASR::asr_t *jvar_ast =
                ASRUtils::make_Variable_t_util(al, loc,
                    current_scope, s2c(al, jvar_name),
            ...
            ASR::symbol_t *jvar_sym =
                ASR::down_cast<ASR::symbol_t>(jvar_ast);
            current_scope->add_symbol(jvar_name, jvar_sym);
            ASR::expr_t *jvar_expr = ASRUtils::EXPR(
                ASR::make_Var_t(al, loc, jvar_sym));

            // Build target: arr(i) % alloc_member(j) %
            //               sub_member % chain... % leaf
            ASRUtils::ExprStmtDuplicator dup_lp(al);
            dup_lp.success = true;

            // Outer loop: do i = dc_start, dc_end
            ASR::do_loop_head_t lh;
            lh.loc = loc;
            lh.m_v = dup_lp.duplicate_expr(x.m_head[0].m_v);
            lh.m_start =
            ...
            ASR::expr_t *tgt_elem = ASRUtils::EXPR(
                ASR::make_ArrayItem_t(al, loc,
                    ...
                    ASR::arraystorageType::ColMajor, nullptr));
            // arr(i)%alloc_member
            ASR::ttype_t *alloc_stripped =
                ASRUtils::type_get_past_allocatable(alloc_mem_type);
            ASR::expr_t *tgt_alloc_mem = ASRUtils::EXPR(
                ASR::make_StructInstanceMember_t(al, loc,
                    tgt_elem, alloc_mem_ext,
                    alloc_mem_type, nullptr));
            // arr(i)%alloc_member(j)
            ASR::ttype_t *alloc_elem_type =
                ASRUtils::type_get_past_array(alloc_stripped);
            ASR::array_index_t *jidx =
                al.allocate<ASR::array_index_t>(1);
            jidx[0].loc = loc;
            jidx[0].m_left = nullptr;
            jidx[0].m_right = jvar_expr;
            jidx[0].m_step = nullptr;
            ASR::expr_t *tgt_expr = ASRUtils::EXPR(
                ASR::make_ArrayItem_t(al, loc,
                    tgt_alloc_mem, jidx, 1, alloc_elem_type,
                    ASR::arraystorageType::ColMajor, nullptr));
            // arr(i)%alloc_member(j)%sub_member
            tgt_expr = ASRUtils::EXPR(
                ASR::make_StructInstanceMember_t(al, loc,
                    tgt_expr, sub_mem_ext,
                    sub_mem_type, nullptr));
            // chain
            for (auto &cp : chain) {
                tgt_expr = ASRUtils::EXPR(
                    ASR::make_StructInstanceMember_t(al, loc,
            ...
            // Inner loop: do j = 1, size(arr(i)%alloc_member)
            ASR::expr_t *tgt_alloc_mem_dup =
                dup_lp.duplicate_expr(tgt_alloc_mem);
            ASR::expr_t *inner_end = ASRUtils::EXPR(
                ASR::make_ArraySize_t(al, loc,
                    tgt_alloc_mem_dup, nullptr,
                    int_type, nullptr));
            ASR::do_loop_head_t jlh;
            jlh.loc = loc;
            jlh.m_v = dup_lp.duplicate_expr(jvar_expr);
            jlh.m_start = ASRUtils::EXPR(
                ASR::make_IntegerConstant_t(al, loc, 1,
                    int_type, ASR::integerbozType::Decimal));
            jlh.m_end = inner_end;
            jlh.m_increment = nullptr;

            Vec<ASR::stmt_t*> jbody;
            jbody.reserve(al, 1);
            jbody.push_back(al, guard);

            ASR::stmt_t *jloop = ASRUtils::STMT(
                ASR::make_DoLoop_t(al, loc, nullptr,
                    jlh, jbody.p, jbody.n,
                    nullptr, 0));

            // Outer loop body: just the inner j loop
            Vec<ASR::stmt_t*> lb;
            lb.reserve(al, 1);
            lb.push_back(al, jloop);
            ...
            if (!ASR::is_a<ASR::Struct_t>(*deeper_sym)) continue;
            chain.push_back({sub_ext, sub_mv->m_type});
            emit_prealloc_deep_through_alloc_member(
                al, loc, current_scope,
                ASR::down_cast<ASR::Struct_t>(deeper_sym),
                sub_ctor_arg, x, tgt_sym, tgt_elem_type,
                alloc_mem_ext, alloc_mem_type,
                sub_mem_ext, sub_mem_type,
                chain, pre_launch_stmts);
            chain.pop_back();
        }
    }
...
public:
    ...
    void inline_matmul_stmts(ASR::stmt_t** &body, size_t &n_body) {
        ...
        for (size_t si = 0; si < n_body; si++) {
            ...
            // Detect MatMul: either directly as the RHS, or inside
            // nested RealBinOps (e.g., z = matmul(w, a) * b * c).
            // We collect a chain of element-wise operations to apply
            // after the matmul computation.
            struct BinopChainEntry {
                ASR::binopType op;
                ASR::expr_t *other;
                bool matmul_is_left;
            };
            ...
            std::vector<BinopChainEntry> binop_chain;

            // Recursively search for MatMul through nested RealBinOps
            std::function<bool(ASR::expr_t*)> find_matmul;
            find_matmul = [&](ASR::expr_t *expr) -> bool {
                if (ASR::is_a<ASR::ArrayPhysicalCast_t>(*expr))
                    expr = ASR::down_cast<ASR::ArrayPhysicalCast_t>(
                        expr)->m_arg;
                if (ASR::is_a<ASR::IntrinsicArrayFunction_t>(*expr)) {
                    auto *f = ASR::down_cast<
                        ASR::IntrinsicArrayFunction_t>(expr);
                    if (static_cast<ASRUtils::IntrinsicArrayFunctions>(
                ...
                if (ASR::is_a<ASR::RealBinOp_t>(*expr)) {
                    ...
                    if (ASR::is_a<ASR::ArrayPhysicalCast_t>(*right))
                        right = ASR::down_cast<ASR::ArrayPhysicalCast_t>(
                            right)->m_arg;
                    if (find_matmul(left)) {
                        binop_chain.push_back(
                            {rbop->m_op, rbop->m_right, true});
                        return true;
                    }
                    if (find_matmul(right)) {
                        binop_chain.push_back(
                            {rbop->m_op, rbop->m_left, false});
                        return true;
                    }
                }
                return false;
            };
            find_matmul(asgn->m_value);
            ...
            if (overload_id == 2 && rank_a == 2 && rank_b == 1) {
                ...
                i_body.reserve(al, 2 + binop_chain.size());                i_body.push_back(al, ASRUtils::STMT(
                    ASR::make_Assignment_t(al, loc, c_i, zero,
                        nullptr, false, false)));
                i_body.push_back(al,
                    make_do_loop(var_k, k_start, k_end, k_body));

                for (auto &entry : binop_chain) {
                    ASR::expr_t *other_op = entry.other;
                    ...
                    ASR::expr_t *other_i = make_section_item(
                        other_op, {var_i});
                    ASR::expr_t *lhs = entry.matmul_is_left
                        ? c_i : other_i;
                    ASR::expr_t *rhs = entry.matmul_is_left
                        ? other_i : c_i;
                    ASR::expr_t *combined = ASRUtils::EXPR(
                        ASR::make_RealBinOp_t(al, loc, lhs, entry.op,
                ...
                j_body.reserve(al, 2 + binop_chain.size());
    ...
    void visit_DoConcurrentLoop(const ASR::DoConcurrentLoop_t &x) {
        ...
        // Into:       arr(i)%x = src(i)
        {
            ...
            for (size_t bi = 0; bi < body_copy.n; bi++) {
                ...
                // with allocatable sub-members (at any depth).
        ...
        // Recursively decompose struct-to-struct assignments where
        // the struct contains allocatable sub-members at any depth.
        // After StructConstructor decomposition, assignments like
        // dst(i)%y = src(i) may remain where b_t has nested
        // allocatables (e.g., b_t%x is a_t with allocatable v).
        // These must be further decomposed into per-member assignments
        // until only leaf-level (primitive or allocatable array)
        // assignments remain, so the Metal kernel can use flat
        // buffers for the data.
        {
            ...
            while (changed) {
                ...
                for (size_t bi = 0; bi < body_copy.n; bi++) {
                    ...
                    ASR::Assignment_t *asgn =
                        ASR::down_cast<ASR::Assignment_t>(stmt);
                    ASR::ttype_t *val_type =
                        ASRUtils::expr_type(asgn->m_value);
                    ASR::ttype_t *val_base =
                        ASRUtils::extract_type(val_type);
                    if (!ASR::is_a<ASR::StructType_t>(*val_base)) {
                        new_body.push_back(al, stmt);
                        continue;
                    }
                    // Get the Struct_t from the target expression.
                    // The target must be a StructInstanceMember whose
                    // member variable has a m_type_declaration pointing
                    // to the Struct symbol.
                    ...
                    ASR::Struct_t *st =
                        ASR::down_cast<ASR::Struct_t>(dt_sym);
                    if (!struct_needs_assignment_decomposition(st)) {
                        new_body.push_back(al, stmt);
                        continue;
                    }
                    // Decompose into per-member assignments
                    ASRUtils::ExprStmtDuplicator dup(al);
                    dup.success = true;
                    for (size_t m = 0; m < st->n_members; m++) {
                        ASR::symbol_t *ms =
                        ...
                        ASR::Variable_t *mv =
                            ASR::down_cast<ASR::Variable_t>(ms);
                        ASR::expr_t *tgt_dup =
                            dup.duplicate_expr(asgn->m_target);
                        ASR::expr_t *val_dup =
                            dup.duplicate_expr(asgn->m_value);
                        ASR::expr_t *tgt_mem = ASRUtils::EXPR(
                            ASR::make_StructInstanceMember_t(
                                al, loc, tgt_dup, mem_ext,
                                mv->m_type, nullptr));
                        ASR::expr_t *val_mem = ASRUtils::EXPR(
                            ASR::make_StructInstanceMember_t(
                                al, loc, val_dup, mem_ext,
                                mv->m_type, nullptr));
                        ASR::stmt_t *ma = ASRUtils::STMT(
                            ASR::make_Assignment_t(al, loc,
                                tgt_mem, val_mem,
        ...
        // Pre-allocate allocatable struct members for whole-struct
        // assignments like b(i) = a(i) inside the do concurrent body.
        // When the source element has an allocated member but the
        // target element does not, the Metal buffer for the target
        // member gets size 0 and the kernel writes out of bounds.
        // For each allocatable array member, emit a pre-launch loop:
        //   do k = dc_start, dc_end
        //     allocate(tgt(k)%member(size(src(k)%member)))
        //   end do
        for (size_t si = 0; si < x.n_body; si++) {
            ASR::stmt_t *stmt = x.m_body[si];
            ASR::stmt_t **stmts_to_scan = &stmt;
            ...
            for (size_t sj = 0; sj < n_stmts_to_scan; sj++) {
                ...
                if (!ASR::is_a<ASR::Var_t>(*src_ai->m_v)) continue;
                ASR::symbol_t *tgt_sym =
                ...
                for (size_t m = 0; m < st->n_members; m++) {
                    ...
                    ASR::ttype_t *mi =
                        ASRUtils::type_get_past_allocatable(mv->m_type);
                    if (ASR::is_a<ASR::Array_t>(*mi)) {
                        has_alloc = true;
                        break;
                    }
                }
                if (!has_alloc) continue;
                if (n_dims != 1) continue;
                ...
                for (size_t m = 0; m < st->n_members; m++) {
                    ...
                    // Find the ExternalSymbol in current_scope
                    // that refers to this member
                    ASR::symbol_t *mem_ext = nullptr;
                    for (auto &scope_item :
                            current_scope->get_scope()) {
                        if (!ASR::is_a<ASR::ExternalSymbol_t>(
                    ...
                    ASR::expr_t *src_member = ASRUtils::EXPR(
                        ASR::make_StructInstanceMember_t(al, loc,
                            src_elem, mem_ext,
                            mv->m_type, nullptr));
                    ASR::expr_t *src_size = ASRUtils::EXPR(
                        ASR::make_ArraySize_t(al, loc,
                            src_member, nullptr,
                            int_type_pa, nullptr));

                    // Build: allocate(tgt_arr(k)%member(src_size))
                    ASR::alloc_arg_t aa;
                    aa.loc = loc;
                    aa.m_a = tgt_member;
                    aa.n_dims = 1;
                    ...
                    // Guard with: if (.not. allocated(tgt(k)%member))
        ...
        for (size_t si = 0; si < x.n_body; si++) {
            ...
            for (size_t sj = 0; sj < n_stmts_to_scan; sj++) {
                ...
                // Pre-allocate nested allocatable sub-members of
                // allocatable array member elements.  Handles patterns
                // like:
                //   arr(i) = top_t([mid_t(data(i))])
                // where items is allocatable array of mid_t, and
                // mid_t%x%v needs pre-allocation so that the Metal
                // buffer offsets are correctly computed.
                // The ArrayConstructor may have been extracted to a
                // temp variable by array_struct_temporary pass.
                for (size_t m = 0; m < st->n_members; m++) {
                    ASR::symbol_t *mem_sym2 =
                        st->m_symtab->get_symbol(st->m_members[m]);
                    if (!mem_sym2 ||
                            !ASR::is_a<ASR::Variable_t>(*mem_sym2))
                        continue;
                    ASR::Variable_t *mv2 =
                        ASR::down_cast<ASR::Variable_t>(mem_sym2);
                    if (!ASRUtils::is_allocatable(mv2->m_type)) continue;
                    ASR::ttype_t *mi2 =
                        ASRUtils::type_get_past_allocatable(mv2->m_type);
                    if (!ASR::is_a<ASR::Array_t>(*mi2)) continue;
                    ASR::ttype_t *elem_inner =
                        ASRUtils::type_get_past_array(mi2);
                    if (!ASR::is_a<ASR::StructType_t>(
                            *ASRUtils::extract_type(elem_inner)))
                        continue;
                    if (!mv2->m_type_declaration) continue;
                    ASR::symbol_t *elem_sym =
                        ASRUtils::symbol_get_past_external(
                            mv2->m_type_declaration);
                    if (!ASR::is_a<ASR::Struct_t>(*elem_sym)) continue;
                    ASR::Struct_t *elem_struct =
                        ASR::down_cast<ASR::Struct_t>(elem_sym);
                    if (!struct_needs_assignment_decomposition(
                            elem_struct))
                        continue;

                    // Get the StructConstructor arg for this member
                    if (m >= sc->n_args) continue;
                    ASR::expr_t *ctor_arg2 = sc->m_args[m].m_value;
                    if (!ctor_arg2) continue;

                    // The arg might be an ArrayConstructor (direct)
                    // or a Var (extracted to temp by earlier pass).
                    // In either case, find the inner StructConstructor
                    // to extract source data for pre-allocation sizes.
                    ASR::StructConstructor_t *inner_sc = nullptr;

                    if (ASR::is_a<ASR::ArrayConstructor_t>(
                            *ctor_arg2)) {
                        ...
                        if (ac->n_args > 0 &&
                            ...
                            inner_sc =
                                ASR::down_cast<
                                    ASR::StructConstructor_t>(
                                    ac->m_args[0]);
                        }
                    } else if (ASR::is_a<ASR::Var_t>(*ctor_arg2)) {
                        // Search entire DC body for assignment to
                        // temp array element with StructConstructor RHS
                        ASR::symbol_t *temp_sym =
                            ASR::down_cast<ASR::Var_t>(
                                ctor_arg2)->m_v;
                        for (size_t bsi = 0;
                                bsi < x.n_body && !inner_sc;
                                bsi++) {
                            ASR::stmt_t *bstmt = x.m_body[bsi];
                            ASR::stmt_t **bstmts = &bstmt;
                            size_t nbstmts = 1;
                            if (ASR::is_a<ASR::BlockCall_t>(
                                    *bstmt)) {
                                ASR::BlockCall_t *bbc =
                                    ASR::down_cast<
                                        ASR::BlockCall_t>(bstmt);
                    ...
                    if (!inner_sc) continue;

                    // Find or create ExternalSymbol for member
                    ASR::symbol_t *mem_ext2 = nullptr;
                    for (auto &scope_item :
                            current_scope->get_scope()) {
                        if (!ASR::is_a<ASR::ExternalSymbol_t>(
                                *scope_item.second)) continue;
                        ASR::ExternalSymbol_t *es =
                            ASR::down_cast<ASR::ExternalSymbol_t>(
                                scope_item.second);
                        if (ASRUtils::symbol_get_past_external(
                                es->m_external) == mem_sym2) {
                            mem_ext2 = scope_item.second;
                            break;
                        }
                    }
                    if (!mem_ext2) {
                    ...
                    // For each non-allocatable struct member of
                    // elem_struct that has nested allocatables,
                    // emit pre-allocation using the inner SC's
                    // source args.
                    for (size_t sm = 0;
                            sm < elem_struct->n_members; sm++) {
                        ASR::symbol_t *sub_sym =
                            elem_struct->m_symtab->get_symbol(
                                elem_struct->m_members[sm]);
                        if (!sub_sym ||
                        ...
                        if (!ASR::is_a<ASR::StructType_t>(
                                *sub_mt))
                            continue;
                        if (!sub_mv->m_type_declaration)
                            continue;
                        ASR::symbol_t *sub_inner_sym =
                            ASRUtils::symbol_get_past_external(
                                sub_mv->m_type_declaration);
                        if (!ASR::is_a<ASR::Struct_t>(
                                *sub_inner_sym))
                            continue;
                        ASR::Struct_t *sub_inner_st =
                            ASR::down_cast<ASR::Struct_t>(
                                sub_inner_sym);
                        if (!struct_has_allocatable_deep(
                                sub_inner_st))
                            continue;
                        if (sm >= inner_sc->n_args) continue;
                        ASR::expr_t *sub_ctor_arg =
                            inner_sc->m_args[sm].m_value;
                        if (!sub_ctor_arg) continue;

                        // Find or create ExternalSymbol for sub member
                        ASR::symbol_t *sub_ext = nullptr;
                        for (auto &scope_item :
                                current_scope->get_scope()) {
                            if (!ASR::is_a<
                        ...
                        emit_prealloc_deep_through_alloc_member(
                            al, loc, current_scope,
                            sub_inner_st, sub_ctor_arg,
                            x, tgt_sym, elem_type,
                            mem_ext2, mv2->m_type,
                            sub_ext, sub_mv->m_type,
                            chain2, pre_launch_stmts);
                    }
                }
                ...
                //   arr(i) = outer_t(x=src(i))
                // where the allocatable members may be nested
                // arbitrarily deep through non-allocatable struct
                // intermediaries (e.g., outer_t%x%y%v).
                for (size_t m = 0; m < st->n_members; m++) {
                    ASR::symbol_t *mem_sym =
                        st->m_symtab->get_symbol(st->m_members[m]);
                    if (!mem_sym ||
