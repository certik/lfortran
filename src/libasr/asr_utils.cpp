...
bool use_overloaded_assignment(ASR::expr_t* target, ASR::expr_t* value,
    ...
    if(!sym) {
        if( ASR::is_a<ASR::StructType_t>(*target_type) && !ASRUtils::is_class_type(target_type) ) {
            ...
            ASR::Struct_t* temp_struct = target_struct;
            while (sym == nullptr && temp_struct->m_parent != nullptr) {
                temp_struct = ASR::down_cast<ASR::Struct_t>(
                    ASRUtils::symbol_get_past_external(temp_struct->m_parent));
                sym = temp_struct->m_symtab->resolve_symbol("~assign");
            }
            expr_dt = target;
            ...
            ASR::Struct_t* temp_struct = value_struct;
            while (sym == nullptr && temp_struct->m_parent != nullptr) {
                temp_struct = ASR::down_cast<ASR::Struct_t>(
                    ASRUtils::symbol_get_past_external(temp_struct->m_parent));
                sym = temp_struct->m_symtab->resolve_symbol("~assign");
